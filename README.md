...
## Scripts
* [getEmptyNamespaces.sh](getEmptyNamespaces.sh): Loop over all namespaces in a cluster and find empty ones.
...
* [getKubernetesResources.sh](getKubernetesResources.sh): Get all pods resources requests, limits and actual usage per container in a CSV format with values normalized.
* [getRestartingPod.sh](getRestartingPod.sh): Get all pods (all or single namespace) that have restarts detected in one or more containers. Formatted in CSV.
